...
int main(){
    ...
    for(int i = 0; i < n; i++){
        if(pthread_create(clientes+1, NULL, (void*) cliente, (void *)&m)){
            printf("Hilo fallido %d creacion cliente/n", i);
            exit(EXIT_FAILURE);
        }
    }
    for(int i = 0; i < m->modelos_disponibles; i++){
        if(pthread_create(proveedores+1, NULL, (void*) proveedor, (void *)&m)){
            printf("Hilo fallido %d creacion prove/n", i);
            exit(EXIT_FAILURE);
        }
    }
    for(int i = 0; i < n; i++){
        if(pthread_join(clientes[i], NULL)){
            printf("Hilo fallido %d recibir cliente/n", i);
            exit(EXIT_FAILURE);
        }
    }
    for(int i = 0; i < m->modelos_disponibles; i++){
        if(pthread_join(proveedores[i], NULL)){
            printf("Hilo fallido %d recibir prove/n", i);
            exit(EXIT_FAILURE);
        }
    }
